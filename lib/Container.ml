...
let add =
  foreign "ges_container_add"
    (ptr t_typ @-> ptr Timeline_element.t_typ @-> returning bool)

(* let edit =
 *   foreign "ges_container_edit" (ptr t_typ @-> ptr List.t_typ @-> int32_t @-> Edit_mode.t_view @-> Edge.t_view @-> uint64_t @-> returning (bool)) *)
let get_children =
  foreign "ges_container_get_children"
    (ptr t_typ @-> bool @-> returning (ptr List.t_typ))

let remove =
  foreign "ges_container_remove"
    (ptr t_typ @-> ptr Timeline_element.t_typ @-> returning bool)
